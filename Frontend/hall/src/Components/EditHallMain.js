...
const EditHallMain = ({_id, name, description, isAvailable, location, phone, rentPerDay, image, carousalImages }) => {
  ...
  const PaystackPayment = (e) => {
    ...
    paystack.newTransaction({
        ...
        onSuccess(transaction){
            let message = `Payment Complete! for ${name}, Reference ${transaction.reference}` 
            alert(message)
...
// Delete a Hall From the Admin
    const deleteHall= async()=>{
        try {
            if(window.confirm( `Do You want to delete ${name}`)){
                await axios.delete(`/api/v1/deleteHall/${_id}`)
            }   
        } catch (error) {
            console.log(error)
        }
    ...
    <>
        ...
        <Modal show={show1} onHide={handleClose1}>
          ...
          <Modal.Footer>
            ...
            <Button variant="primary" onClick={PaystackPayment} >
             Pay Now
            </Button>
          </Modal.Footer>
        </Modal>
...
<Button   style={{marginLeft: "5px", backgroundColor:"green", border:"1px solid green"}}> <a href={location} style={{textDecoration: "none", color: "white"}}  target="_blank">Locate Hall</a></Button>
<>
...
<Button variant="primary"  style={{marginLeft: "6px", marginTop:"5px",}}> <Link  style={{textDecoration:"none", color:"white"}} to={`/editHall/${_id}`}> Edit Hall </Link>

</Button>

<Button variant="danger" onClick={()=>deleteHall(_id)} style={{marginLeft: "11px", marginTop:"5px"}}>
Delete Hall
</Button>
